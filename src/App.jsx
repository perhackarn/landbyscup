...
import { db, auth, collection, addDoc, updateDoc, deleteDoc, doc, onSnapshot, query, orderBy, runTransaction, signOut, onAuthStateChanged } from './firebase';
...
function App() {
  ...
  const [shooters, setShooters] = React.useState([]);
  ...
  React.useEffect(() => {
    ...
    return onSnapshot(q, (snapshot) => {
      setShooters(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    });
  }, []);
  React.useEffect(() => {
...
function Competitions({ competitions, user }) {
  ...
  const [skiljemal, setSkiljemal] = React.useState([]);
  const [editId, setEditId] = React.useState(null);
  const [editName, setEditName] = React.useState("");
  const [editDate, setEditDate] = React.useState("");
  const [editSkiljemal, setEditSkiljemal] = React.useState([]);
    ...
    <section>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        ...
        <button onClick={addCompetition}
          ...
          disabled={!user}>Lägg till</button>
      </div>
...
function Shooters({ shooters, user }) {
  ...
  const [klass, setKlass] = React.useState("öppen");
  const [confirmation, setConfirmation] = React.useState(null);

  const [editId, setEditId] = React.useState(null);
  const [editName, setEditName] = React.useState("");
  const [editClub, setEditClub] = React.useState("");
  const [editKlass, setEditKlass] = React.useState("öppen");
  ...
  return (
    <section>
      ...
      {confirmation && (
        <div className="bg-green-100 border border-green-300 text-green-800 px-4 py-3 rounded-lg relative mb-4 shadow-sm" role="alert">
          ...
          <button onClick={() => setConfirmation(null)} className="absolute top-0 bottom-0 right-0 px-4 py-3">
            <svg className="fill-current h-6 w-6 text-green-600" role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><title>Close</title><path d="M14.348 14.849a1.2 1.2 0 0 1-1.69..." /></svg>
          </button>
        </div>
      )}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <input value={name} onChange={e=>setName(e.target.value)} placeholder="Namn"
          className="border border-primary-300 p-3 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors"/>
        <input value={club} onChange={e=>setClub(e.target.value)} placeholder="Ort"
          className="border border-primary-300 p-3 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors"/>
        ...
        <button onClick={addShooter}
          className="bg-green-600 hover:bg-green-700 text-white px-4 py-3 rounded-lg font-medium transition-colors shadow-sm">Lägg till</button>
      </div>
      ...
      <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Sök skytt, ort eller startnummer..."
...
function Scores({ shooters, competitions, user, scores }) {
  ...
  const [searchShooter, setSearchShooter] = React.useState("");
  const [filteredShooters, setFilteredShooters] = React.useState([]);
  const [shooterId, setShooterId] = React.useState("");
  ...
  const [existingScores, setExistingScores] = React.useState([]);
  ...
  // Hämta redan registrerade scores för vald tävling/station/skytt
  React.useEffect(() => {
  ...
  // Starta redigera ett befintligt score-dokument
...
try {
  ...
  return (
    <section>
      ...
      {/* Visar redan registrerade skott för det valda (tävling, station, skytt) */}
      <div className="mb-6">
...
function CupResults({ shooters, scores, competitions }) {
  ...
  // Steg 1: Beräkna placering för varje skytt i varje tävling, per klass
  ...
  // Steg 2: Beräkna total cup-poäng för varje skytt
  ...
  klassLista.forEach(klass => {
    ...
    }).filter(s => s.cupTotal < 5000); // Filtrera bort de som inte deltagit alls
  ...
  const exportPDF = () => {
    ...
    klassLista.forEach(klass => {
      if (finalCupResults[klass] && finalCupResults[klass].length > 0) {
        if (startY > 180) { // Check if there's enough space, otherwise add new page
          doc.addPage();
          startY = 20;
        }
